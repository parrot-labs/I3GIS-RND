...
for (int baris = 1; baris <= input.getRowNumbers(); baris++) {
	...
	WebUI.delay(1)
	WebUI.setText(findTestObject('Object Repository/03-SAST/03-Quality Code/Quality Code Filter/Input new Filter'), input.getValue('filter-name-variant', baris))
	WebUI.delay(1)
	WebUI.click(findTestObject('Object Repository/03-SAST/03-Quality Code/Quality Code Filter/Button Create'))
	WebUI.delay(2)
	WebUI.verifyTextPresent(input.getValue('verify-variant', baris), false, FailureHandling.OPTIONAL)
