...
for (int baris = 1; baris <= 1; baris++) {
	...
	if (WebUI.verifyTextPresent('Data Project Not Found', false, FailureHandling.OPTIONAL) == true) {
		...
		if (WebUI.verifyTextPresent(input.getValue('branch', baris), false, FailureHandling.OPTIONAL) == true) {
			WebUI.click(findTestObject('Object Repository/02-Summary Project/02-Detail Project-Create Project/div-'+input.getValue('branch', baris)))
			WebUI.delay(1)
		}else {
		...
		WebUI.delay(1)
		WebUI.click(findTestObject('Object Repository/02-Summary Project/02-Detail Project-Create Project/Div-'+input.getValue('access', baris)))
		WebUI.delay(1)
		...
		WebUI.callTestCase(findTestCase('Test Cases/02-Project/01-Create Project'), [:], FailureHandling.STOP_ON_FAILURE)
