...
for (int baris = 1; baris <= input.getRowNumbers(); baris++) {
	...
	WebUI.delay(1)
	WebUI.verifyTextPresent('Success assign users to this issue', false, FailureHandling.STOP_ON_FAILURE)
	WebUI.takeScreenshot()
	...
	WebUI.click(findTestObject('Object Repository/07-DAST/Issue-Context/Button-Save'))
	WebUI.delay(1)
	WebUI.verifyTextPresent('Success revoke users from this issue', false, FailureHandling.STOP_ON_FAILURE)
	WebUI.takeScreenshot()
	WebUI.refresh()
	WebUI.delay(2)
	WebUI.takeScreenshot()
	WebUI.callTestCase(findTestCase('Test Cases/06-DAST/Issue RISK/HIGH'), [:], FailureHandling.STOP_ON_FAILURE)
	WebUI.callTestCase(findTestCase('Test Cases/06-DAST/Issue RISK/MEDIUM'), [:], FailureHandling.STOP_ON_FAILURE)
	WebUI.callTestCase(findTestCase('Test Cases/06-DAST/Issue RISK/LOW'), [:], FailureHandling.STOP_ON_FAILURE)
	WebUI.callTestCase(findTestCase('Test Cases/06-DAST/Issue RISK/INFO'), [:], FailureHandling.STOP_ON_FAILURE)
	WebUI.refresh()
