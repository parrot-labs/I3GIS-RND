...
for (int baris = 1; baris <= input.getRowNumbers(); baris++) {
	...
	WebUI.verifyTextPresent(input.getValue('alert-username', baris), false, FailureHandling.OPTIONAL)
	WebUI.delay(1)
	WebUI.click(findTestObject('Object Repository/Login/Button Login'))
	WebUI.delay(1)
	WebUI.verifyTextPresent(input.getValue('alert-password', baris), false, FailureHandling.OPTIONAL)
